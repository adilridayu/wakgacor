...
<body class="font-outfit bg-[#F8F9FA] text-secondary flex min-h-screen">
    ...
    <main class="flex-grow ml-72 p-10">
        ...
        <header class="flex justify-between items-center mb-10">
            <div>
                <h1 class="text-3xl font-800 text-secondary tracking-tight">Manajemen <span class="text-primary">Subkategori</span></h1>
                <p class="text-gray-400 mt-1">Hierarki detail untuk pengelompokan produk yang lebih presisi.</p>
            </div>
            
            <div class="flex gap-4">
                <form class="flex gap-2">
                    <div class="relative">
                        <input class="bg-white border border-gray-100 rounded-2xl px-12 py-3.5 outline-none focus:ring-4 focus:ring-primary/10 transition-all text-sm w-64 shadow-sm" type="search" placeholder="Cari subkategori..." name="search">
                        <i class="fas fa-search absolute left-5 top-1/2 -translate-y-1/2 text-gray-300"></i>
                    </div>
                </form>
                <a href="create_subcategory.php" class="w-12 h-12 bg-primary text-secondary rounded-2xl flex items-center justify-center shadow-xl shadow-primary/20 hover:scale-[1.05] transition-all">
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-gray-50/50">
                            ...
                            <th class="px-6 py-6 text-[10px] font-800 text-gray-400 uppercase tracking-[2px]">Subkategori</th>
                            <th class="px-6 py-6 text-[10px] font-800 text-gray-400 uppercase tracking-[2px]">Kategori Induk</th>
                    ...
                    <tbody class="divide-y divide-gray-50">
                        ...
                        $sql = "SELECT
                                ...
                                FROM category_subcategories
                                    INNER JOIN subcategories ON category_subcategories.subcategory_id = subcategories.id
                        ...
                        foreach ($subcategories as $subcategory) {
                            ...
                            <tr class="hover:bg-gray-50/50 transition-colors group">
                                <td class="px-8 py-6">
                                    <span class="text-xs font-800 text-gray-400">#<?= $subcategory["id"] ?></span>
                                </td>
                                <td class="px-6 py-6">
                                    <form class="flex items-center gap-2" method="POST" action="../../../api/admin/subcategories.php">
                                        <input type="hidden" name="action" value="update">
                                        <input type="hidden" name="id" value="<?= $subcategory['id'] ?>">
                                        <input class="input-field-inline w-40" type="text" name="name" value="<?= htmlspecialchars($subcategory['name'], ENT_QUOTES) ?>">
                                ...
                                <td class="px-6 py-6">
                                    <form class="flex items-center gap-2" method="POST" action="../../../api/admin/subcategories.php">
                                        <input type="hidden" name="action" value="update">
                                        <input type="hidden" name="id" value="<?= $subcategory['id'] ?>">
                                        <select class="input-field-inline w-40 appearance-none" name="category">
                                            <option value="<?= $subcategory['category_id'] ?>"><?= $subcategory['category'] ?></option>
                                ...
                                <td class="px-6 py-6">
                                    <p class="text-xs font-semibold text-gray-500"><?= date('d M Y', strtotime($subcategory["created_at"])) ?></p>
                                </td>
                                <td class="px-6 py-6">
                                    <div class="flex items-center justify-center">
                                        <form method="POST" action="../../../api/admin/subcategories.php" onsubmit="return confirm('Hapus subkategori ini?');">
                                            <input type="hidden" name="id" value="<?= $subcategory["id"] ?>">
