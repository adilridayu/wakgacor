...
<head>
    ...
    <title>Manajemen Pengguna - Wak Gacor Store</title>
    <!-- Google Fonts -->
...
<body class="font-outfit bg-[#F8F9FA] text-secondary flex min-h-screen">
    ...
    <main class="flex-grow ml-72 p-10">
        ...
        <header class="flex justify-between items-center mb-10">
            <div>
                <h1 class="text-3xl font-800 text-secondary tracking-tight">Manajemen <span class="text-primary">Pengguna</span></h1>
                <p class="text-gray-400 mt-1">Daftar pelanggan dan akun terdaftar di platform Anda.</p>
            </div>
            
            <a href="create_user.php" class="px-6 py-3.5 bg-primary text-secondary font-800 rounded-2xl transition-all duration-500 hover:bg-white hover:scale-[1.05] shadow-xl shadow-primary/20 flex items-center gap-2">
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-gray-50/50">
                            ...
                            <th class="px-6 py-6 text-[10px] font-800 text-gray-400 uppercase tracking-[2px]">Info Pengguna</th>
                            <th class="px-6 py-6 text-[10px] font-800 text-gray-400 uppercase tracking-[2px]">Kontak</th>
                            <th class="px-6 py-6 text-[10px] font-800 text-gray-400 uppercase tracking-[2px]">Terdaftar</th>
                    ...
                    <tbody class="divide-y divide-gray-50">
                        ...
                        foreach ($users as $user) {
                            ...
                            <tr class="hover:bg-gray-50/50 transition-colors group">
                                <td class="px-8 py-6">
                                    <span class="text-xs font-800 text-gray-400">#<?= $user["id"] ?></span>
                                </td>
                                <td class="px-6 py-6">
                                    <div class="flex items-center gap-3">
                                        <div class="w-10 h-10 rounded-full bg-secondary flex items-center justify-center text-primary font-bold text-xs border border-primary/20">
                                            <?= strtoupper(substr($user["first_name"], 0, 1) . substr($user["last_name"], 0, 1)) ?>
                                        </div>
                                        <div>
                                            <p class="font-bold text-secondary text-sm group-hover:text-primary transition-colors"><?= htmlspecialchars($user["first_name"] . " " . $user["last_name"]) ?></p>
                                            <p class="text-[10px] text-gray-400 mt-0.5"><?= htmlspecialchars($user["email"]) ?></p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-6">
                                    <div class="flex flex-col gap-1">
                                        <span class="text-xs font-semibold text-secondary"><?= htmlspecialchars($user["phone_number"]) ?></span>
                                        <span class="text-[10px] text-gray-400">Lahir: <?= htmlspecialchars($user["dob"]) ?></span>
                                    </div>
                                </td>
                                <td class="px-6 py-6">
                                    <p class="text-xs font-semibold text-gray-500"><?= date('d M Y', strtotime($user["created_at"])) ?></p>
                                </td>
                                <td class="px-6 py-6">
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        <form method="POST" action="../../../api/admin/users.php" class="inline" onsubmit="return confirm('Hapus pengguna ini?');">
