...
<body class="font-outfit bg-[#F8F9FA] text-secondary flex min-h-screen">
    ...
    <main class="flex-grow ml-72 p-10">
        ...
        <header class="flex justify-between items-center mb-10">
            <div>
                <h1 class="text-3xl font-800 text-secondary tracking-tight">Tambah <span class="text-primary">Kategori</span></h1>
                <p class="text-gray-400 mt-1">Buat kategori baru untuk mengelompokkan produk Anda.</p>
            </div>
            
            <a href="categories.php" class="px-6 py-3.5 bg-white text-gray-400 font-bold rounded-2xl transition-all duration-300 hover:bg-secondary hover:text-white shadow-sm flex items-center gap-2">
        ...
        <div class="max-w-2xl bg-white rounded-[40px] shadow-sm border border-gray-100 p-10">
            <form action="../../../api/admin/categories.php" method="POST" class="space-y-8">
                <input type="hidden" value="create" name="action">

                <div>
                    <label class="form-label" for="name">Nama Kategori</label>
                    <div class="relative group">
                        <input class="form-input" id="name" type="text" placeholder="Contoh: Akun Premium" name="name" required>
                        <i class="fas fa-tag absolute right-6 top-1/2 -translate-y-1/2 text-gray-300 group-hover:text-primary transition-colors"></i>
                    </div>
                </div>
                ...
                <div class="flex justify-end pt-4">
                    <button type="submit" class="px-10 py-4 bg-primary text-secondary font-800 rounded-2xl transition-all duration-500 hover:bg-secondary hover:text-white hover:scale-[1.02] shadow-xl shadow-primary/20 flex items-center gap-3 w-full justify-center">
                        <i class="fas fa-plus-circle text-lg"></i> BUAT KATEGORI
                    </button>
                </div>
            </form>
        </div>
